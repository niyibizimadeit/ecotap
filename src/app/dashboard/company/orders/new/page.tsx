...
/* ── Price helpers (corporate pricing) ── */
function getPricePerCard(currency: Currency): number {
  return currency === "USD" ? CARD_PRICES.corporate : usdToRwf(CARD_PRICES.corporate);
}
...
/* ── Main component ── */
export default function CompanyNewOrderPage() {
  ...
  function validateStep1(): boolean {
    if (!form.design_id) { setErrors({ design: "Please select a card design." }); return false; }
  ...
  function validateStep2(): boolean {
    ...
    if (form.quantity < 1 || form.quantity > 100) errs.quantity = "Quantity must be between 1 and 100.";
    setErrors(errs);
  ...
  async function handlePlaceOrder() {
    ...
    if (result.success && result.data) {
      ...
      router.push(`/dashboard/company/orders/success?order=${result.data.id}`);
    } else {
      setLoading(false);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Bulk corporate pricing at $28/card. Physical cards shipped to your office."
        action={
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          ...
          {step === 1 && (
            <div className="animate-fade-up">
              <SectionHeading number={1} title="Choose your card design" subtitle="Select the design for your team's cards." />
              <DesignGallery
                selected={form.design_id}
                onSelect={id => { setForm(f => ({ ...f, design_id: id })); setErrors({}); }}
                designs={designs}
              />
              {errors.design && <p className="text-sm text-red-600 mt-3">{errors.design}</p>}
              <div className="mt-6">
          ...
          {step === 2 && (
            <div className="animate-fade-up space-y-5">
              <SectionHeading number={2} title="Shipping details" subtitle="Where should we send your cards?" />
              ...
              <div className="rounded-2xl border p-5" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
                ...
                <div className="flex items-center gap-4">
                  <button onClick={() => setForm(f => ({ ...f, quantity: Math.max(1, f.quantity - 1) }))}
                    ...
                    style={{ borderColor: "rgba(6,78,59,0.15)", color: "#064E3B" }}>−</button>
                  ...
                  <button onClick={() => setForm(f => ({ ...f, quantity: Math.min(100, f.quantity + 1) }))}
                    className="w-10 h-10 rounded-xl border text-lg font-medium transition-all hover:bg-emerald-pale"
                    style={{ borderColor: "rgba(6,78,59,0.15)", color: "#064E3B" }}>+</button>
                  <div className="flex-1">
                    <input type="range" min={1} max={100} value={form.quantity}
                      onChange={e => setForm(f => ({ ...f, quantity: Number(e.target.value) }))}
                      className="w-full accent-emerald-bright" />
                    <div className="flex justify-between text-xs text-ink-light mt-0.5"><span>1</span><span>100</span></div>
                  </div>
                </div>
              ...
              <div className="rounded-2xl border p-5 space-y-4" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
                ...
                <Input label="Street address" placeholder="KG 123 St, Gasabo" required value={form.shipping_address.street} onChange={setAddress("street")} error={errors.street} />
                <div className="grid grid-cols-2 gap-4">
                  <Input label="City" placeholder="Kigali" required value={form.shipping_address.city} onChange={setAddress("city")} error={errors.city} />
                  <Input label="Country" required value={form.shipping_address.country} onChange={setAddress("country")} error={errors.country} />
                </div>
                <Input label="Postal code" placeholder="Optional" value={form.shipping_address.postal_code ?? ""} onChange={setAddress("postal_code")} />
                <Textarea label="Delivery notes" placeholder="Landmark, gate code, best time to deliver..." value={form.shipping_address.notes ?? ""} onChange={setAddress("notes")} className="min-h-[72px]" hint="Optional" />
              </div>

              <div className="flex gap-3">
                <Button variant="secondary" size="lg" leftIcon={<ArrowLeft className="h-4 w-4" />} onClick={back}>Back</Button>
                <Button variant="primary" size="lg" rightIcon={<ArrowRight className="h-4 w-4" />} onClick={next} className="flex-1">Continue to payment</Button>
              </div>
            </div>
          )}
          ...
          {step === 3 && selectedDesign && (
            <div className="animate-fade-up space-y-5">
              <SectionHeading number={3} title="Complete your payment" subtitle="Pay via mobile money or bank transfer, then upload your receipt." />
              ...
              <div className="rounded-2xl border p-5" style={{ backgroundColor: form.currency === "RWF" ? "#ECFDF5" : "#EFF6FF", borderColor: "rgba(6,78,59,0.08)" }}>
                ...
                <div className="flex items-baseline justify-between">
                  <div>
                    <p className="font-serif text-3xl font-bold text-emerald-deep">{formatCurrency(totalPrice, form.currency)}</p>
                    <p className="text-sm text-ink-light mt-0.5">
                      {form.quantity} card{form.quantity !== 1 ? "s" : ""} × {formatCurrency(pricePerCard, form.currency)} each (corporate rate)
                    </p>
                  </div>
                  {form.currency === "RWF" && (
                    <p className="text-xs text-ink-light text-right">≈ ${CARD_PRICES.corporate * form.quantity} USD</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <Button variant="secondary" size="lg" leftIcon={<ArrowLeft className="h-4 w-4" />} onClick={back}>Back</Button>
                <Button variant="primary" size="lg" rightIcon={<ArrowRight className="h-4 w-4" />} onClick={next} className="flex-1">Review order</Button>
              </div>
            </div>
          )}
          ...
          {step === 4 && selectedDesign && (
            <div className="animate-fade-up space-y-5">
              <SectionHeading number={4} title="Review your order" subtitle="Check everything looks right before placing." />
              <OrderSummary design={selectedDesign} quantity={form.quantity} address={form.shipping_address} />

              <div className="rounded-2xl border p-4 space-y-3" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm font-semibold text-emerald-deep">{formatCurrency(totalPrice, form.currency)}</p>
                </div>
                <div className="flex items-center justify-between">
              ...
              <div className="rounded-2xl border p-4 text-sm" style={{ backgroundColor: "#FEF3C7", borderColor: "rgba(146,64,14,0.15)", color: "#92400E" }}>
                ...
                <p className="text-xs leading-relaxed" style={{ color: "rgba(146,64,14,0.8)" }}>
                  Your payment screenshot will be reviewed. Once verified, your order goes into production. Cards are printed and shipped within 3–5 days.
                </p>
              </div>
              ...
              <div className="flex gap-3">
                <Button variant="secondary" size="lg" leftIcon={<ArrowLeft className="h-4 w-4" />} onClick={back}>Back</Button>
                <Button variant="primary" size="lg" className="flex-1" loading={loading} onClick={handlePlaceOrder}
                  leftIcon={!loading ? <Package className="h-4 w-4" /> : undefined}>
                  {loading ? "Placing order…" : "Place order"}
                </Button>
              </div>
              <p className="text-xs text-ink-light text-center">Your order will be reviewed and approved by the EcoTap team before production begins.</p>
            </div>
          )}
        </div>
        ...
        <div className="hidden lg:block">
          <div className="sticky top-8 space-y-4">
            ...
            {selectedDesign ? (
              <div className="rounded-2xl border p-4" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
                <div className="w-full h-20 rounded-xl mb-3" style={{ backgroundColor: selectedDesign.accent }} />
              ...
              <div className="rounded-2xl border p-4 text-center" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
                <p className="text-xs text-ink-light">No design selected yet</p>
              </div>
            )}
            <div className="rounded-2xl border px-4 py-3 flex items-center justify-between" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
            ...
            {step >= 3 && (
              <div className="rounded-2xl border px-4 py-3" style={{ backgroundColor: "#ECFDF5", borderColor: "rgba(6,78,59,0.08)" }}>
                <p className="text-xs text-ink-light mb-0.5">Total ({form.currency})</p>
                <p className="font-serif text-xl font-semibold text-emerald-deep">{formatCurrency(totalPrice, form.currency)}</p>
              </div>
            )}
            {form.shipping_address.city && (
              <div className="rounded-2xl border px-4 py-3" style={{ backgroundColor: "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
            ...
            {step >= 3 && (
              <div className="rounded-2xl border px-4 py-3" style={{ backgroundColor: form.screenshot_url ? "#ECFDF5" : "#FEF9EF", borderColor: "rgba(6,78,59,0.08)" }}>
