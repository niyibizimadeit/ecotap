...
async function ContactsContent() {
  ...
  if (!result.success || !result.data) {
    return <EmptyState icon="⚠️" title="Could not load contacts" description="Please try again later." />;
  }

  const contacts = result.data;

  if (contacts.length === 0) {
    return <EmptyState icon="📭" title="No contacts yet" description="When visitors submit their info on your card page, they'll appear here." />;
  }
